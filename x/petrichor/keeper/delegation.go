...
// Delegate is the entry point for delegators to delegate petrichor assets to validators
...
func (k Keeper) Delegate(ctx sdk.Context, delAddr sdk.AccAddress, validator types.PetrichorValidator, coin sdk.Coin) (*sdk.Dec, error) {
	// Check if asset is whitelisted as an petrichor asset
	asset, found := k.GetAssetByDenom(ctx, coin.Denom)
	if !found {
		return nil, status.Errorf(codes.NotFound, "asset with denom: %s does not exist in petrichor whitelist", coin.Denom)
	}

	// Check and send delegated tokens into the petrichor module address
...
func (k Keeper) Redelegate(ctx sdk.Context, delAddr sdk.AccAddress, srcVal types.PetrichorValidator, dstVal types.PetrichorValidator, coin sdk.Coin) (*time.Time, error) {
...
func (k Keeper) Undelegate(ctx sdk.Context, delAddr sdk.AccAddress, validator types.PetrichorValidator, coin sdk.Coin) (*time.Time, error) {
	asset, found := k.GetAssetByDenom(ctx, coin.Denom)
	if !found {
		return nil, status.Errorf(codes.NotFound, "Asset with denom: %s does not exist", coin.Denom)
	}
	...
	// Remove tokens and shares from the petrichor asset
...
func (k Keeper) GetDelegation(ctx sdk.Context, delAddr sdk.AccAddress, validator types.PetrichorValidator, denom string) (d types.Delegation, found bool) {
...
func (k Keeper) SetValidator(ctx sdk.Context, val types.PetrichorValidator) {
	store := ctx.KVStore(k.storeKey)
	key := types.GetPetrichorValidatorInfoKey(val.GetOperator())
	vb := k.cdc.MustMarshal(val.PetrichorValidatorInfo)
	store.Set(key, vb)
}

func (k Keeper) SetValidatorInfo(ctx sdk.Context, valAddr sdk.ValAddress, val types.PetrichorValidatorInfo) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) ValidateDelegatedAmount(delegation types.Delegation, coin sdk.Coin, val types.PetrichorValidator, asset types.PetrichorAsset) (shares sdk.Dec, err error) {
...
func (k Keeper) upsertDelegationWithNewTokens(ctx sdk.Context, delAddr sdk.AccAddress, validator types.PetrichorValidator, coin sdk.Coin, asset types.PetrichorAsset) (types.Delegation, sdk.Dec) {
...
func (k Keeper) reduceDelegationShares(ctx sdk.Context, delAddr sdk.AccAddress, validator types.PetrichorValidator, coin sdk.Coin, shares sdk.Dec, delegation types.Delegation) {
...
func (k Keeper) updateValidatorShares(ctx sdk.Context, validator types.PetrichorValidator, delegationShares sdk.DecCoins, validatorShares sdk.DecCoins, isAdd bool) {
	if isAdd {
...
// GetPetrichorBondedAmount returns the total amount of bonded native tokens that are not in the
// unbonding pool
func (k Keeper) GetPetrichorBondedAmount(ctx sdk.Context, delegator sdk.AccAddress) math.Int {
	bonded := sdk.ZeroDec()
