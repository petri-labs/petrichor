...
// UpdatePetrichorAsset updates the petrichor asset with new params
...
func (k Keeper) UpdatePetrichorAsset(ctx sdk.Context, newAsset types.PetrichorAsset) error {
	...
	if !newAsset.RewardWeight.Equal(asset.RewardWeight) {
		k.IteratePetrichorValidatorInfo(ctx, func(valAddr sdk.ValAddress, info types.PetrichorValidatorInfo) bool {
...
func (k Keeper) RebalanceHook(ctx sdk.Context, assets []*types.PetrichorAsset) error {
...
// the difference.
func (k Keeper) RebalanceBondTokenWeights(ctx sdk.Context, assets []*types.PetrichorAsset) (err error) {
	moduleAddr := k.accountKeeper.GetModuleAddress(types.ModuleName)
	petrichorBondAmount := k.GetPetrichorBondedAmount(ctx, moduleAddr)

	nativeBondAmount := k.stakingKeeper.TotalBondedTokens(ctx).Sub(petrichorBondAmount)
	bondDenom := k.stakingKeeper.BondDenom(ctx)

	unbondedValidatorShares := sdk.NewDecCoins()
	var bondedValidators []types.PetrichorValidator

	// Iterate through all petrichor validators to remove those that are unbonded.
	// Unbonded validators will be ignored when rebalancing.
	k.IteratePetrichorValidatorInfo(ctx, func(valAddr sdk.ValAddress, info types.PetrichorValidatorInfo) bool {
		var validator types.PetrichorValidator
		validator, err = k.GetPetrichorValidator(ctx, valAddr)
		if err != nil {
			return true
		}
...
func (k Keeper) SetAsset(ctx sdk.Context, asset types.PetrichorAsset) {
	store := ctx.KVStore(k.storeKey)
	b := k.cdc.MustMarshal(&asset)
	store.Set(types.GetAssetKey(asset.Denom), b)
}

func (k Keeper) GetAllAssets(ctx sdk.Context) (assets []*types.PetrichorAsset) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetAssetByDenom(ctx sdk.Context, denom string) (asset types.PetrichorAsset, found bool) {
	store := ctx.KVStore(k.storeKey)
...
// DeductAssetsHook is called periodically to deduct from an petrichor asset (calculated by take_rate).
// The interval in which assets are deducted is set in module params
func (k Keeper) DeductAssetsHook(ctx sdk.Context, assets []*types.PetrichorAsset) (sdk.Coins, error) {
...
// DeductAssetsWithTakeRate Deducts an petrichor asset using the take_rate
// The deducted asset is distributed to the fee_collector module account to be redistributed to stakers
func (k Keeper) DeductAssetsWithTakeRate(ctx sdk.Context, lastClaim time.Time, assets []*types.PetrichorAsset) (sdk.Coins, error) {
...
func (k Keeper) SetRewardWeightChangeSnapshot(ctx sdk.Context, asset types.PetrichorAsset, val types.PetrichorValidator) {
...
func (k Keeper) RewardWeightChangeHook(ctx sdk.Context, assets []*types.PetrichorAsset) {
	for _, asset := range assets {
		...
		k.QueueAssetRebalanceEvent(ctx)
		k.UpdatePetrichorAsset(ctx, *asset)
	}
}
