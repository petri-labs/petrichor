...
func TestRewardPoolAndGlobalIndex(t *testing.T) {
	...
	err = app.PetrichorKeeper.AddAssetsToRewardPool(ctx, mintPoolAddr, val1, sdk.NewCoins(sdk.NewCoin("stake", sdk.NewInt(2000_000))))
	require.Error(t, err)
	...
	_, err = app.PetrichorKeeper.Delegate(ctx, user2, val1, sdk.NewCoin(PETRICHOR_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)
	assets = app.PetrichorKeeper.GetAllAssets(ctx)
	err = app.PetrichorKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	err = app.PetrichorKeeper.AddAssetsToRewardPool(ctx, mintPoolAddr, val1, sdk.NewCoins(sdk.NewCoin("stake2", sdk.NewInt(4000_000))))
	require.NoError(t, err)
...
func TestClaimRewards(t *testing.T) {
	...
	asset, _ = app.PetrichorKeeper.GetAssetByDenom(ctx, PETRICHOR_2_TOKEN_DENOM)
	require.Equal(t,
		sdk.NewInt(1000_000),
		val1.TotalTokensWithAsset(asset).TruncateInt(),
	)
	...
	coins, err = app.PetrichorKeeper.ClaimDelegationRewards(ctx, user1, val1, PETRICHOR_TOKEN_DENOM)
	require.NoError(t, err)
	...
	_, err = app.PetrichorKeeper.ClaimDelegationRewards(ctx, user2, val1, PETRICHOR_TOKEN_DENOM)
	require.Error(t, err)

	// User 2 claims rewards
	coins, err = app.PetrichorKeeper.ClaimDelegationRewards(ctx, user2, val1, PETRICHOR_2_TOKEN_DENOM)
	require.NoError(t, err)
	...
	delegation, found := app.PetrichorKeeper.GetDelegation(ctx, user1, val1, PETRICHOR_TOKEN_DENOM)
	require.True(t, found)
	require.Equal(t, indices, types.NewRewardHistories(delegation.RewardHistory))

	delegation, found = app.PetrichorKeeper.GetDelegation(ctx, user2, val1, PETRICHOR_2_TOKEN_DENOM)
	require.True(t, found)
	require.Equal(t, indices, types.NewRewardHistories(delegation.RewardHistory))
}
...
func TestClaimRewardsWithMultipleValidators(t *testing.T) {
	...
	app.PetrichorKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.DefaultParams(),
		Assets: []types.PetrichorAsset{
			types.NewPetrichorAsset(PETRICHOR_TOKEN_DENOM, sdk.NewDec(2), sdk.NewDec(0), startTime),
	...
	_, err = app.PetrichorKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(PETRICHOR_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)
	...
	coins, err = app.PetrichorKeeper.ClaimDelegationRewards(ctx, user2, val2, PETRICHOR_2_TOKEN_DENOM)
	require.NoError(t, err)
...
func TestClaimRewardsAfterRewardsRatesChange(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(PETRICHOR_TOKEN_DENOM, sdk.NewInt(10_000_000)),
		sdk.NewCoin(PETRICHOR_2_TOKEN_DENOM, sdk.NewInt(10_000_000)),
	))
	...
	// New delegations
	app.PetrichorKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(PETRICHOR_TOKEN_DENOM, sdk.NewInt(1000_000)))
	app.PetrichorKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(PETRICHOR_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	assets := app.PetrichorKeeper.GetAllAssets(ctx)
	err = app.PetrichorKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	err = app.PetrichorKeeper.UpdatePetrichorAsset(ctx, types.NewPetrichorAsset(PETRICHOR_TOKEN_DENOM, sdk.NewDec(10), sdk.NewDec(0), ctx.BlockTime()))
	require.NoError(t, err)
	assets = app.PetrichorKeeper.GetAllAssets(ctx)
	err = app.PetrichorKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	val1, _ = app.PetrichorKeeper.GetPetrichorValidator(ctx, valAddr1)
	iter := app.PetrichorKeeper.IterateWeightChangeSnapshot(ctx, PETRICHOR_TOKEN_DENOM, valAddr1, 0)
	...
	rewards1, err := app.PetrichorKeeper.ClaimDelegationRewards(ctx, user1, val1, PETRICHOR_TOKEN_DENOM)
	require.NoError(t, err)
	require.Equal(t, sdk.NewInt(5_000_000+1_666_666), rewards1.AmountOf(bondDenom))

	rewards2, err := app.PetrichorKeeper.ClaimDelegationRewards(ctx, user2, val2, PETRICHOR_2_TOKEN_DENOM)
	require.NoError(t, err)
...
func TestRewardClaimingAfterRatesDecay(t *testing.T) {
	...
	rewardStartDelay := app.PetrichorKeeper.RewardDelayTime(ctx)
	...
	_, err = app.PetrichorKeeper.Delegate(ctx, addrs[1], val0, sdk.NewCoin(PETRICHOR_2_TOKEN_DENOM, sdk.NewInt(5_000_000)))
	require.NoError(t, err)
	//
	assets := app.PetrichorKeeper.GetAllAssets(ctx)
	...
	app.PetrichorKeeper.AddAssetsToRewardPool(ctx, addrs[0], val0, sdk.NewCoins(sdk.NewCoin(bondDenom, sdk.NewInt(1000_000))))
	assets = app.PetrichorKeeper.GetAllAssets(ctx)
	...
	coins, err := app.PetrichorKeeper.ClaimDelegationRewards(ctx, addrs[1], val0, PETRICHOR_TOKEN_DENOM)
	coins2, err := app.PetrichorKeeper.ClaimDelegationRewards(ctx, addrs[1], val0, PETRICHOR_2_TOKEN_DENOM)
