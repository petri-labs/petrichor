...
func ValidatorSharesInvariant(k keeper.Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		infos := k.GetAllPetrichorValidatorInfo(ctx)
		...
		for _, asset := range assets {
			if !validatorShares[asset.Denom].IsNil() && !asset.TotalValidatorShares.Equal(validatorShares[asset.Denom]) {
				broken = true
				msg += fmt.Sprintf("broken petrichor validator share invariance: \n"+
...
func DelegatorSharesInvariant(k keeper.Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		for val, assets := range delegatorShares {
			...
			if err != nil {
				msg = fmt.Sprintf("petrichor validator address invalid\n")
				broken = true
				break
			}
			info, found := k.GetPetrichorValidatorInfo(ctx, valAddr)
			if !found {
				msg = fmt.Sprintf("petrichor validator info for %s not found\n", val)
				broken = true
				break
			}
			...
			for denom, amount := range assets {
				if !shares.AmountOf(denom).Equal(amount) {
					msg += fmt.Sprintf("broken petrichor delegation share invariance: \n"+
