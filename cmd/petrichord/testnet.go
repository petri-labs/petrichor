...
Example:
	petrichord testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.5.20
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "petrichord", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagStartingIPAddress, "192.168.5.20", "Starting IP address (192.168.5.20 results in persistent peers list ID0@192.168.5.20:46656, ID1@192.168.5.21:46656, ...)")
...
Example:
	petrichord testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initGenFiles(
	...
	// PETRICHOR
	var petrichorGenState petrichortypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[petrichortypes.ModuleName], &petrichorGenState)
	petrichorParams := petrichorGenState.GetParams()
	petrichorParams.TakeRateClaimInterval = time.Second * 30
	petrichorParams.RewardDelayTime = time.Second * 30
	petrichorGenState.Params = petrichorParams
	appGenState[petrichortypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&petrichorGenState)
